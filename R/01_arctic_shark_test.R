...
# load the shark data 
teeth <- read.table(file = './data/sharks/Arctic_shark_PO4.txt', 
                    header = TRUE)
...
temperature_prior_vector <- runif(100000, 8, 13)
d18Ofw_prior_vector      <- runif(100000, -20.3, -17.5)
...
parameters$parameters |> 
  add_column(taxon = 'arctic sharks') |> 
  filter(iteration > parameters$burn) |> 
  select(-iteration) |> 
  write_csv(file = './results/arctic_sharks_posterior.csv')
