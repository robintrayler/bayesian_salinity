...
teeth        <- read.csv(file = './data/sharks/Carcharias sp.csv', header = TRUE) 
...
parameters$parameters |> 
  add_column(taxon = 'Carcharias sp') |> 
  filter(iteration > parameters$burn) |> 
  select(-iteration) |> 
  write_csv(file = './results/Carcharias_sp_posterior.csv')

parameters$CI |> 
  add_column(taxon = 'Carcharias sp') |> 
  write_csv(file = './results/Carcharias_sp_CI.csv')