...
teeth        <- read.csv(file = './data/sharks/Striatolamia.csv', header = TRUE) 
...
parameters$parameters |> 
  ...
  filter(iteration > parameters$burn) |> 
  select(-iteration) |> 
  write_csv(file = './results/Striatolamia_posterior.csv')

parameters$CI |> 
  add_column(taxon = 'Striatolamia') |> 
  write_csv(file = './results/Striatolamia_CI.csv')