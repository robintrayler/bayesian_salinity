...
teeth        <- read.csv(file = './data/sharks/Brachycarcharias.csv', header = TRUE) 
...
temperature_prior_vector <- as.vector(t_prior$Temp)
d18Ofw_prior_vector      <- as.vector(d18Ofw_prior$d18Of)
...
parameters$parameters |> 
  add_column(taxon = 'Brachycarcharias') |> 
  filter(iteration > parameters$burn) |> 
  select(-iteration) |> 
  write_csv(file = './results/Brachycarcharias_posterior.csv')

parameters$CI |> 
  add_column(taxon = 'Brachycarcharias') |> 
  write_csv(file = './results/Brachycarcharias_CI.csv')